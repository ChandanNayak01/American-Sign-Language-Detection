...
import argparse
import itertools
import time  # Added for timing logic
...
def main():
    ...
    current_text = ""          # Current text being built
    current_sign = ""          # Current detected sign
    last_sign = ""             # Last stable sign
    sign_start_time = 0        # When we started seeing the current sign
    no_hands_start_time = 0    # When we last saw no hands
    hands_present = False      # Whether hands are currently detected
    stable_threshold = 1.5     # Seconds needed for a sign to be considered stable
    space_threshold = 5.0      # Seconds of no hands to add a space
    sign_added = False         # Flag to track if the current sign has been added
    ...
    while True:
        ...
        # Track hand presence for word formation logic
        if results.multi_hand_landmarks is None:
            # No hands detected
            if hands_present:
                # Hands were present before but not now
                hands_present = False
                no_hands_start_time = time.time()
                current_sign = ""  # Reset current sign when hand disappears
            elif not hands_present:
                # Still no hands
                time_without_hands = time.time() - no_hands_start_time
                if time_without_hands > space_threshold and current_text and not current_text.endswith(" "):
                    # Add space after the word if no hands for space_threshold seconds
                    current_text += " "
                    sign_added = False
        else:
            # Hands detected
            if not hands_present:
                # Hands weren't present before but are now
                hands_present = True
            
            # Reset no hands timer when hands are detected
            no_hands_start_time = time.time()
        ...
        if mode == 2:
            ...
            detected_sign = ""  # The sign detected in this frame
            ...
            if results.multi_hand_landmarks is not None:
                ...
                if detected_sign != current_sign:
                    ...
                    # Same sign still detected
                    sign_duration = time.time() - sign_start_time
                    if sign_duration > stable_threshold and not sign_added:
                        # Sign is stable enough to be added to the text
                        current_text += current_sign
                        sign_added = True
                        last_sign = current_sign

            # Draw the current text on screen
            debug_image = draw_text_box(debug_image, current_text)
...
def draw_text_box(image, text):
    # Display the current text in a box at the top of the screen
    box_color = (64, 64, 64)  # Dark gray background
    ...
    cv.rectangle(
        image,
        (margin, margin),
        (image.shape[1] - margin, margin + text_size[1] + 20),
        box_color,
        -1
    )
    ...
    cv.putText(
        image,
        text,
        (margin + 10, margin + text_size[1] + 5),
        font,
        font_scale,
        text_color,
        font_thickness,
